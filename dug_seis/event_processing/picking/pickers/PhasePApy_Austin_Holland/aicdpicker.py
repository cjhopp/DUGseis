...
#       print "It took %f s to threshold" %(time.time()-now)
    ...
    # picks
#       now=time.time()
    scnl, picks, trigger, snr, aics = summary.pick_ident()
...
#       print "It took %f s to polarity" %(time.time()-now)
    return scnl, picks, polarity, snr, uncertainty, summary, aics
    #else:
...
class AICDSummary():
  ...
  def pick_ident(self):
    ...
    for i in range(len(trigger)):
      ...
      while True:
        if self.summary[index] > self.summary[index - 1]:
          ...
          break
      # Woah woah! Need more than 3 sig figs!
    ...
    return scnl, picks, trigger, SNR, aics
  ...
  def uncertainty(self):
    ...
    scnl, picks, trigger, SNR, aics = self.pick_ident()
    dt = self.stats.delta
  ...
  def plot_summary(self, filename=None):
    ...
    dt = self.stats.delta
    t = np.arange(0, self.stats.npts) * dt
    ...
    ax2.plot(t, self.summary, c='k', label='Normalized CF')
    ax3 = ax2.twinx()
    ax3.plot(t, self.thres, '--', linewidth=2.0, c='k', label='Threshold')
    ...
    for i in range(len(picks)):
      ax2.plot([(picks[i]-self.tr.stats.starttime),
                (picks[i]-self.tr.stats.starttime)], [0, np.max(self.summary)],
               'r--', label='Picks')
      ax.plot([(picks[i]-self.tr.stats.starttime),
               (picks[i]-self.tr.stats.starttime)], [0, np.max(self.tr.data)],
               'r--', label='Picks')
      ax2.text((picks[i]-self.tr.stats.starttime), 0.5 * np.max(self.summary),
